...
WARNING: this script assumes a DEDICATED VM. Step 3 frees ports 80, 443 and
8000, which stops anything already serving on them. To deploy onto a VM that
already hosts other sites, do not run this: instead start only the API
container on a spare loopback port and add a server block to the existing
nginx. The README section "Live deployment" shows that arrangement, and is
how https://buptestapi.ahbab.dev is actually running.
"""
